...
- **2026-07-03 addition:** FR-37 gained a gate-time consequence that warns when the token ledger crosses a researcher-set soft limit in `config.yaml`. This is *not* the O6 trigger firing — it's deterministic reporting at a checkpoint that already exists (FR-37's decision block), same category as the rest of this list's "guidance, not contract" framing. It exists specifically to accumulate the "cost ledger shows a state chronically overrunning" evidence O6 names as its own adoption trigger, so the two items are sequenced, not in tension. Live enforcement (rejecting or blocking on budget) remains out of scope until O6 fires on real data.
